...
class Ellipsoid_MV(Problem):
    ...
    def get_solution_quality(self, solution: dict) -> (float, bool):
        ...
        continuous_function_value = 0
        discrete_function_value = 0

        for i in range(1, self._continuous_dimension+1):

            continuous_function_value += (100 ** ((i - 1) / (self._dimension - 1)) * continuous_variables[i-1]) ** 2

        for i in range(1, self._discrete_dimension+1):

            discrete_function_value += (100 ** ((i + self._continuous_dimension - 1) / (self._dimension - 1)) *  discrete_variables[i-1]) ** 2
