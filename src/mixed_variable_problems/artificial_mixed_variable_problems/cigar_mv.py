...
class Cigar_MV(Problem):
    ...
    def get_solution_quality(self, solution: dict) -> (float, bool):
        ...
        continuous_function_value = continuous_variables[0] ** 2
        discrete_function_value = 0

        for i in range(self._continuous_dimension - 1):
            continuous_function_value += 10 ** 4 * continuous_variables[i + 1] ** 2

        for j in range(self._discrete_dimension):
            discrete_function_value += 10 ** 4 * discrete_variables[j] ** 2
