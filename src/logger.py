...
class Logger:
    ...
    def __init__(self, path="output/", mode=MODE_RUN, suffix_number=0, algorithm='hsppbo') -> None:
        ...
        Class variables:
            test_dynamic (bool, optional):  Wether or not the logger should handle this as a test of multiple problem dynamics, and save aggregated info. Defaults to None.
    ...
    def init_folder(self) -> int:
        ...
        starting with 1 if the folder is new or counting up until the highes number is found.
    ...
    def next_dynamic(self, dynamic_num: int, **param) -> None:
        ...
        for k in param.keys():
            self.folder_info[self.sub_name + str(dynamic_num)].setdefault(k, []).append(param[k])
    ...
    def log_iteration(self, it_num: int, func_evals: int, swap_num: int, reaction: bool, best_solution_quality: float, best_solution: list) -> None:
        """
        ...
        Args:
            ...
            func_evals (int): Number of function evaluations needed for calculating the solution
            swap_num (int): Number of swaps that happened during that iteration
            reaction (bool): If the change handling reaction was triggered or not 
            best_solution_quality (float): The iteration best solution quality
            best_solution (list): The iteration best solution
        """
        ...
        self.run_list.append(
            [it_num, runtime, func_evals, swap_num, reaction, best_solution_quality, best_solution])

        self.run_io.write("%d;%0.3f;%d;%d;%r;%f;%s\n" %
                          (it_num, runtime, func_evals, swap_num, reaction, best_solution_quality, best_solution))
