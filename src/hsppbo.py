...
class HSPPBO:
    ...
    def __init__(self, problem: Problem, logger: Logger, pers_best=0.075, pers_prev=0.075, parent_best=0.01,
        ...
        for i in range(0, self.max_iteration_count):
            ...
            if self.problem.check_dynamic_change(i):
                ...
                for sce in range(0, self.sce_count):
                    pp_quality = self.problem.get_solution_quality(self.tree.get_solution(sce)[0])
                    pb_quality = self.problem.get_solution_quality(self.tree.get_best_solution(sce)[0])
            ...
            for sce in self.tree.all_nodes_top_down:
                ...
                sce_solution_quality = self.tree.get_best_solution(sce)[1]
                # get the solution quality of the sce and its parent
                parent_sce = self.tree.get_parent(sce)
                parent_solution_quality = self.tree.get_best_solution(parent_sce)[1]
            ...
            self.logger.log_iteration(i, self.sce_count*(i+1), swap_count, detection_pause_count == 0, best_solution_quality)
    ...
    def tau(self, populations: tuple, i: int, k: int) -> float:
        ...
        """
        pop_range_sum = ((self.w_pers_prev if self.is_solution_subset((i, k), populations[0]) else 0)
                         + (self.w_pers_best if self.is_solution_subset((i, k), populations[1]) else 0)
                         + (self.w_parent_best if self.is_solution_subset((i, k), populations[2]) else 0))
