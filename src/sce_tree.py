...
class SCETree:
    ...
    def __init__(self, init_solution: Problem.init_solution, node_count=13, child_count=3):
        ...
        Args:
            ...
            child_count (int): Max number of child nodes for each node (k-ary tree). Defaults to 3, so a ternary tree
        """
    ...
    def init_tree(self):
        ...
        for i in range(0, self.nodes):
            ...
            self.tree.create_node("SCE_"+str(i), i, parent=p, data=SCENode(pers_best, pb_quality, pers_prev, pp_quality))
    ...
    def get_populations(self, nid: int) -> tuple[dict[int]]:
        ...
        if self.tree.root == nid:
            return ({k: v for v, k in enumerate(self.tree.get_node(nid).data.pers_prev)},
                    {k: v for v, k in enumerate(self.tree.get_node(nid).data.pers_best)},
                    {k: v for v, k in enumerate(self.tree.get_node(nid).data.pers_best)})
        else:
            pid = self.tree.parent(nid).identifier
            return ({k: v for v, k in enumerate(self.tree.get_node(nid).data.pers_prev)},
                    {k: v for v, k in enumerate(self.tree.get_node(nid).data.pers_best)},
                    {k: v for v, k in enumerate(self.tree.get_node(pid).data.pers_best)})
