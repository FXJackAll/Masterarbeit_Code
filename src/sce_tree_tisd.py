...
class SCETree_TISD:
    ...
    def update_node(self, nid: int, solution: tuple, solution_quality: float) -> None:
        ...
        # print("in sce_tree_tisd update_node: " + str(nd.pb_quality))
    ...
    def reset_pers_best(self, level=0) -> None:
        ...
        for n in nodes:
            # print("sce_tree_tisd reset")
            # n.data.pers_best, n.data.pb_quality = self.solution_generator()
            n.data.pers_best = self.solution_generator()[0]
            n.data.pb_quality = self.solution_generator()[1][0]
    ...
    def get_populations(self, nid: int) -> tuple[dict[int]]:
        ...
        if self.tree.root == nid:
            ...
            return (self.tree.get_node(nid).data.pers_prev,
                    self.tree.get_node(nid).data.pers_best,
                    self.tree.get_node(pid).data.pers_best)
