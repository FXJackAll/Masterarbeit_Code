...
class TSP(Problem):
    ...
    # Fix rounding issue for euclidean 2D weight calc, always output float
    ...
    def init_solution(self) -> tuple[tuple[int], int]:
        """
        Initialize a new possible solution for the given TSP instance.
    ...
    def check_dynamic_change(self, iteration_count: int):
        ...
        if self.dynamic:
            if iteration_count % self.dynamic_frequency == 0 and iteration_count > self.min_iteration_count:
                ...
                # get random permutation of all city nodes
                ...
                city_pairs = [list(a) for a in np.array_split(cities, len(cities)/2)]
    ...
    def get_distance_matrix(self) -> np.ndarray:
        ...
        Returns:
            np.ndarray: ND-array with the problem instance equivalent distance matrix
        """
