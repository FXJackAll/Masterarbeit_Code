...
class H_Het_P_ACO_R_Very_Simple(Search_Algorithm):
    ...
    def construct_solution(self, search_parameters: dict) -> dict:
        ...
        linear_div_inflations = search_parameters['linear_div_inflations']
        h_probabilities = search_parameters['h_probabilities']

        linear_div_inflation = 1
        h_probability = 0.05

        if solution_creating_behavior == 'weak-heterogeneous':

            match sce_index:

                case 0:

                    linear_div_inflation = linear_div_inflations[0]
                    h_probability = h_probabilities[0]

                case sce_index if 1 <= sce_index <= 3:

                    linear_div_inflation = linear_div_inflations[1]
                    h_probability = h_probabilities[1]

                case sce_index if 4 <= sce_index <= 12:

                    linear_div_inflation = linear_div_inflations[2]
                    h_probability = h_probabilities[2]

                case sce_index if 13 <= sce_index <= 39:

                    linear_div_inflation = linear_div_inflations[3]
                    h_probability = h_probabilities[3]
        ...
        solution_choice = choice(['calculated', 'random'], 1, p=[1-h_probability, h_probability])
        ...
        for variable in range(len(natural_numbers_populations[0])):
            ...
            match solution_choice:
                ...
                case 'random':
                    ...
                    new_natural_numbers_solution_coordinates[variable] = [
                        round(uniform(left_boundary[i], right_boundary[i])) for i in range(len(linear_div_inflated_natural_numbers))]
        ...
        for variable in range(len(continuous_populations[0])):
            ...
            match solution_choice:
                ...
                case 'calculated':
                    ...
                    # print("in aco_r_very_simple: " + str(left_boundary))
                    # print("in h_p_aco_r_very_simple: " + str(solution_choice))
                ...
                case 'random':
                    ...
                    new_continuous_solution_coordinates[variable] = [
                        uniform(left_boundary[i], right_boundary[i]) for i in range(len(linear_div_inflated))]
