    ...
    def __init__(self, con_pr_name: str,
        ...
        if load_instance:
            ...
            with open(os.path.join(self._problem_path, self._con_pr_name + ".json")) as f:
                ...
                self._alpha = self._instance["alpha"]
                self._A = self._instance["A"]
                self._P = self._instance["P"]
    ...
    def get_solution_quality(self, solution: dict) -> (float, bool):
        ...
        for i in range(4):
            ...
            for j in range(3):
                ...
                term -= self._A[i][j] * (continuous_variables[j] - self._P[i][j]) ** 2

            function_value -= self._alpha[i] * exp(term)
