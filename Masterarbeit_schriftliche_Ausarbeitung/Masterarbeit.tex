		...
		\begin{lstlisting}[language=json, caption=Paraboloid 5 Dim gemischte Variablen, label=subsubsec:Testfunktionen:Konstruierte_Testfunktionen:JSON:Paraboloid5Dim11MV]
{
	...
	Alle Varianten von \ac{acorvsmv} können mit gemischten Variablen umgehen. Aus Platzgründen wurde in den Tabellen wurde das \glqq -MV\grqq{} weggelassen.
	...
	Wie in den Tabellen \ref{subsec:Vergleich_Von_ACO_R_Und_Iterationen_Von_ACO_R_Very_Simple:tab:Vergleich_Zu_Ameisen_Verwandte_Algorithmen_Teil_1}, \ref{subsec:Vergleich_Von_ACO_R_Und_Iterationen_Von_ACO_R_Very_Simple:tab:Vergleich_Zu_Ameisen_Verwandte_Algorithmen_Teil_2} und \ref{subsec:Vergleich_Von_ACO_R_Und_Iterationen_Von_ACO_R_Very_Simple:tab:Vergleich_Zu_Ameisen_Verwandte_Algorithmen_Teil_3} zu sehen, im Gegenteil zu den Testfunktionen, die zum Vergleich zwischen Algorithmen, die probabilistisches Lernen einsetzen, benutzt werden, ist es bei Testfunktionen, die benutzt werden, um Algorithmen, die verwandt zu Ameisenalgorithmen sind, zu testen, nicht mehr eindeutig, welcher Algorithmus der beste ist. So sind ACO\(_\mathbb{R}\) und \ac{acorvs} die besten Algorithmen jeder in jeweils 3 von 8 Probleme und Hom-HPB-ACO\(_\mathbb{R}\)-VS-MV mit NSS-f0 ist der beste Algorithmus in 2 der 8 Probleme. Ferner sind die Algorithmen ACO\(_\mathbb{R}\), \ac{acorvs} und Hom-HPB-ACO\(_\mathbb{R}\)-VS-MV mit NSS-f0 bei den Probleme, wo sie nicht der beste Algorithmus sind, wenn man ihre relative Anzahl an Funktionsauswertungen betrachtet, nahe dem Bestwert, außer bei den Testfunktionen \glqq Griewangk\grqq{} und \glqq Rosenbrock(5)\grqq, wo \ac{acorvs} viel schlechter ist als ACO\(_\mathbb{R}\) und Hom-HPB-ACO\(_\mathbb{R}\)-VS-MV mit NSS-f0 nicht konvergiert. Auf der anderen Seite kann aus der Tabelle~\ref{subsec:Vergleich_Von_ACO_R_Und_Iterationen_Von_ACO_R_Very_Simple:tab:Vergleich_Zu_Ameisen_Verwandte_Algorithmen_Teil_1} entnommen werden, dass die Algorithmen CACO, API und CIAC, dort, wo Ergebnisse in der Literatur berichtet wurden, schlechter bis viel schlechter sind, als die drei genannten Favoriten. Bei den Varianten von \ac{acorvsmv} mit NSS-adapt sind die Ergebnisse gemischt aber da sie bei 4 der 8 Probleme viel schlechter sind, werden sie auch, insgesamt, als schlechter als ACO\(_\mathbb{R}\), \ac{acorvs} und Hom-HPB-ACO\(_\mathbb{R}\)-VS-MV mit NSS-f0 betrachtet.\hfill\newline
	...
	\begin{table}[H]
		...
		\caption{Vergleich zwischen ACO\(_{\mathbb{R}}\), \acl{acorvs}, Varianten von \acl{acorvsmv} und anderen Meta-Heuristiken, die für stetige Probleme adaptiert wurden, basierend auf \textit{Socha und Dorigo}~\cite{socha2008ant} und damit auf \textit{Kern et al., 2004}~\cite{kern2004learning}, Teil 1}
	...
	Dies ist der letzte Vergleich, den in dieser Arbeit betrachten wird. In Abschnitt~\ref{subsec:Vergleich_Algorithmen:Vergleich_Auf_Stetigen_Testfunktionen} wurde gezeigt, dass die Varianten von \ac{acorvsmv} mit stetigen Testfunktionen umgehen können und im Abschnitt~\ref{subsubsec:Vergleich_Algorithmen:Vergleich_Auf_Konstruierten_Testfunktionen_Mit_Gemischten_Variablen} haben wurde gezeigt, dass die Varianten von \ac{acorvsmv} mit konstruierten Testfunktionen mit gemischten Variablen umgehen können. Jetzt wird untersucht, ob die Varianten von \ac{acorvsmv} auch mit einem realen Problem umgehen können.\hfill\newline 
	...
	Wir können jedoch das selbe Ergebnis beobachten, wie bei den konstruierten Testfunktionen mit gemischten Variablen, nämlich, dass die Varianten von \ac{acorvsmv} mit NSS-f0 besser sind als die Varianten von \ac{acorvsmv} mit NSS-adapt. Es kann auch weiter beobachten werden, dass, wenn der gesamte Raum, zum ziehen der Initiallösungen, genommen wird, aber dann nur, mit den Constrains konforme, Lösungen bei der Lösungssuche erlaubt werden, also bei dem Fall \glqq penalty-exact\grqq, keine Iteration von \ac{acorvsmv}, unabhängig der Neustartstrategie, konvergiert. Es wird davon ausgegangen, dass das daran liegt, dass, ausgehend von nur 13 zufälligen Initiallösungen, es sehr schwer ist, eine mit den Constrains konforme Lösung zu generieren, da die Größe des gesamten Suchraums, aus dem die Initiallösungen gezogen werden, im Vergleich zur Größe des Teils des Suchraums, der mit den Constrains konforme Lösungen enthält, viel größer ist, wie in Abbildung~\ref{subsubsec:Testfunktionen:CSD:fig:3D_plot:CSD:filtered_and_unfiltered_search_spaces_combined} zu sehen.
		...
		\caption{Vergleich der Ergebnisse zwischen den Varianten von \acl{acorvsmv} mit Neustartstrategie \(adaptive\) (NSS-adapt)}
	...
	\begin{table}[H]
		...
		\caption{Vergleich der Ergebnisse zwischen den Varianten von \acl{acorvsmv} mit Neustartstrategie \(fixed = 0\) (NSS-f0)}
